...
def fetchvlan():
        ...
        | grep -i segmentation|awk '{{print $4}}';done".format(vlansName),\
        shell=True, stdout=subprocess.PIPE).communicate()[0].strip().split('\n')
        return vlans
...
Ospfv2=ospfv2id-om_cn_vr,Area=area-0.0.1.1,\
Interface=if-0.0.1.1-{}\n".format(i))
...
# SIG OSPF
                remote_conn.send("no ManagedElement=1,Transport=1,Router=0-26-sig_cn_vr,\
                                  Ospfv2=ospfv2id-sig_cn_vr,Area=area-0.0.1.2,\
                                  Interface=if-0.0.1.2-{}\n".format(i))
